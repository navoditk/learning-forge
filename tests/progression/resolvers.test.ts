...
const records = [
  ...
  { id: 'ratio-language', version: '2.0.0', review: { status: 'pending_review' as const } },
  {
    id: 'retired-item',
    version: '1.0.0',
...
describe('active and historical content resolution', () => {
  it('serves the newest reviewed version and excludes retired or pending versions', () => {
    expect(resolveActive(records, 'ratio-language').version).toBe('1.0.0');
  ...
  it('resolves an exact archived version regardless of current serving eligibility', () => {
    expect(resolveHistorical(records, 'ratio-language', '2.0.0')).toMatchObject({
      version: '2.0.0',
