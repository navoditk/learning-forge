...
| Provider terms | Provider, region, retention, training use, subprocessors, moderation, deletion, and approved-data boundary | Security/privacy + legal | Provider decided 2026-09-06 (ADR-0009), adapter implemented and verified 2026-09-06 (ADR-0011) but not yet used for any real session. Region, retention, training-use, subprocessor, and moderation terms still pending. |
...
| Evaluation | Synthetic corpus, severity labels, adjudication owner, leakage/correctness/tone/injection/accessibility cases, baseline report | Quality/evaluation owner | Gate approach decided 2026-09-06 (ADR-0009). Existing 9-case synthetic corpus (one per dimension) run through the real adapter 2026-09-06 (ADR-0011); all automated checks passed. **Still pending**: the product owner's personal read of the actual generated text — the real gate — and a decision on whether to expand toward the ~20-30-case target. |
