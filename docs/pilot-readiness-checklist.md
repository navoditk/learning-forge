...
| Identity and access | Managed identity choice, parent/learner linking, household authorization tests, support-access procedure | Product/engineering + privacy | Decided 2026-09-06 (ADR-0008) and implemented 2026-09-06 (ADR-0010): email/password login (NextAuth v5), verified end-to-end (correct/incorrect credentials, unauthenticated redirects). **Not yet done**: `phase1/service.ts` still resolves data via the synthetic fixture rather than the authenticated household, so login gates access but a real household's own data isn't shown yet; no support-access procedure exists. |
