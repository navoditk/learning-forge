...
| Identity and access | Managed identity choice, parent/learner linking, household authorization tests, support-access procedure | Product/engineering + privacy | Decided 2026-09-06 (ADR-0008), implemented 2026-09-06 (ADR-0010), and **confirmed live 2026-09-13**: email/password login (NextAuth v5), household-scoped data access (`requireHouseholdContext`), verified end-to-end locally with two distinct real households, and now in production — the real account signed in and used the app at `https://learning-forge.onrender.com`. **Still not done**: no support-access procedure exists; no managed identity provider is in front of this (self-managed credentials only). |
...
| Provider terms | Provider, region, retention, training use, subprocessors, moderation, deletion, and approved-data boundary | Security/privacy + legal | Provider decided 2026-09-06 (ADR-0009), adapter implemented, reviewed, and enabled for real sessions 2026-09-06 (ADR-0011). **Still pending**: region, retention, training-use, subprocessor, and moderation contractual terms with Anthropic. |
...
| Evaluation | Synthetic corpus, severity labels, adjudication owner, leakage/correctness/tone/injection/accessibility cases, baseline report | Quality/evaluation owner | Gate approach decided and satisfied 2026-09-06 (ADR-0009/ADR-0011): existing 9-case synthetic corpus (one per dimension) run through the real adapter, all automated checks passed, and the product owner personally reviewed and approved the actual generated text. **Expanded and closed 2026-09-13**: corpus grew to 22 cases (multiple variations per dimension), re-run through the real adapter (22/22 passed), product owner reviewed and approved. The ~20-30-case target is met. |
...
| Persistence | Migration apply/rollback evidence, household scoping, immutable attempts, export/deletion drill | Data/engineering owner | Local migration, rollback, household-scoping, immutable-attempt, household export, and deletion checks pass. Render backup schedule and a synthetic restore drill remain pending; see `docs/backup-recovery.md`. |
...
| Operations | Rate/cost limits, latency target, monitoring, alerting, rollback, incident contacts | Engineering/operations | Budget/latency targets decided 2026-09-06 (ADR-0009). A basic hourly adapter backstop (60 calls/hour), configurable household/session hint limits, and a database recovery runbook were added. **Still pending**: Render backup/restore verification, spend monitoring, billing alerts, and incident contacts. |
