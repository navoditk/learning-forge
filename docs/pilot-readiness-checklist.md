...
| Evaluation | Synthetic corpus, severity labels, adjudication owner, leakage/correctness/tone/injection/accessibility cases, baseline report | Quality/evaluation owner | Gate approach decided and satisfied 2026-09-06 (ADR-0009/ADR-0011): existing 9-case synthetic corpus (one per dimension) run through the real adapter, all automated checks passed, and the product owner personally reviewed and approved the actual generated text. **Still open**: whether to expand toward the ~20-30-case target for a deeper pass before a broader pilot. |
