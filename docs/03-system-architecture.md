...
| **Provider-neutral `TutorModel` port** | `TutorHarness`, `FakeTutorModel`, `AnthropicTutorModel`, `createTutorModel()`, structured move and constructed-response boundaries | Prevents provider SDK types and behavior from leaking into policy, content, student-model, or reporting modules; enables synthetic testing before provider approval | Port, fake adapter, and a real Anthropic adapter (ADR-0011) are implemented — the harness sources real model identity/latency/token metadata from whichever adapter runs, and degrades gracefully (retry-then-fallback) on a real provider error, not just a schema-invalid response. **Enabled** for real sessions after product-owner review: `/api/phase1/hint` uses `createTutorModel()`, gated on `TUTOR_MODEL_PROVIDER=anthropic` (tests/CI always force the fake adapter). Contractual data-processing terms, region, and production credential rotation remain pending |
...
| **Fixed synthetic identity boundary** | Test/CI fixture only (`SYNTHETIC_IDENTITY`, `tests/`) | Keeps every test deterministic and database-independent of real credentials while preserving the same household-ownership checks production identity uses | Implemented for tests/CI only (ADR-0003). Real requests no longer use this fixture at all — `src/phase1/service.ts` and every `/api/phase1/*` route resolve the authenticated household instead (`requireHouseholdContext`, ADR-0010), verified with two distinct real households seeing correctly isolated data |
| **NextAuth v5 (Auth.js) authentication** | `src/auth/`, `src/middleware.ts`, `src/app/login/`, `src/server/household-context.ts`, `scripts/create-parent-account.ts` | Implements ADR-0008's identity decision (email/password, single household) using a widely-audited session/cookie library rather than hand-rolled session code, in an area where mistakes are costly | Implemented (ADR-0010): password hashing, login, middleware-gated routes, and household-scoped data access all work end-to-end, verified locally and in e2e/integration tests. Deployment-specific config (`AUTH_URL`/`AUTH_TRUST_HOST` for the real Render domain) remains for the hosting track |
...
| **Provider-neutral `NotifierPort`** | `buildWeeklyDigest`, `ConsoleNotifier`, `getWeeklyDigest`, `GET /api/phase1/digest`, future real email/push adapter | Mirrors the `TutorModel` port pattern so a parent-communication provider can be chosen later without coupling digest content to a specific vendor | Digest builder (multi-skill), console/fake adapter, and an on-demand route/parent-page button are implemented — a real caller exists, closing the earlier "no caller" gap. No scheduler triggers it automatically (that remains the deferred database-backed job seam below), and no real provider, address, or delivery exists |
| **Versioned skill catalog and planner** | `skillCatalog` (`content/skills/*.json`), `planNextActivities`, `getPlan`, `GET /api/phase1/plan` | Applies the LF-0.5 content-as-versioned-JSON precedent to the skill graph (ADR-0006) rather than adding new Postgres tables; the planner is a pure function so it is testable without a database or scheduler | Skill catalog (19 skills, cycle/prerequisite-integrity checked), the deterministic planner, a Phase 1 route, and an actionable "Recommended next activities" learner-page section are implemented (ADR-0007) — selecting a recommended item starts a real session for it. No scheduler exists, no diagnostic/placement logic chooses a learner's starting item, and no `ReviewSchedule`/spaced-review timing exists |
| **Managed Node/PostgreSQL deployment** | Future web runtime and managed database for an invite-only pilot | Preserves the local architecture while delegating routine runtime/database operations to a managed platform; vendor choice should follow privacy, region, cost, backup, and operational review | Platform accepted 2026-09-06: Render (ADR-0005), for the single-household pilot scoped in ADR-0008. No deployment or production data path exists yet — region, account setup, and the actual deploy remain follow-up engineering work |
