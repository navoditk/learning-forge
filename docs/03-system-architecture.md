...
| **Versioned skill catalog and planner** | `skillCatalog` (`content/skills/*.json`), `planNextActivities` | Applies the LF-0.5 content-as-versioned-JSON precedent to the skill graph (ADR-0006) rather than adding new Postgres tables; the planner is a pure function so it is testable without a database or scheduler | Skill catalog (19 skills, cycle/prerequisite-integrity checked) and a deterministic planner implemented. No API route, UI, or scheduler calls the planner yet, and no `ReviewSchedule`/spaced-review timing exists |
