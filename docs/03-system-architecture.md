...
| **Fixed synthetic identity boundary** | Local learner session, parent evidence scope, household ownership checks, and synthetic seed data | Allows end-to-end development without authentication credentials or real child data while preserving the ownership checks that production identity must satisfy | Implemented for local/CI only (ADR-0003). Access to the app is now gated by real authentication (below), but `src/phase1/service.ts` still resolves every request against the synthetic fixture rather than the authenticated household — rewiring that is tracked, separately, as the next step |
| **NextAuth v5 (Auth.js) authentication** | `src/auth/`, `src/middleware.ts`, `src/app/login/`, `scripts/create-parent-account.ts` | Implements ADR-0008's identity decision (email/password, single household) using a widely-audited session/cookie library rather than hand-rolled session code, in an area where mistakes are costly | Implemented (ADR-0010): password hashing, login, and middleware-gated routes work end-to-end, verified locally and in e2e. Deployment-specific config (`AUTH_URL`/`AUTH_TRUST_HOST` for the real Render domain) remains for the hosting track; `phase1/service.ts` is not yet wired to the authenticated household |
