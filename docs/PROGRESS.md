...
- Phase: 1 — synthetic journeys across all 5 Grade 6 Math domains, skill graph, planner, an actionable planner UI, an on-demand parent weekly digest, and a basic accessible visual design (now automated-WCAG-AA-checked) covered; content is reviewed and approved (scope noted below); real identity/provider approvals remain pending
- Branch: `feature/automated-accessibility-checks`
- Repository state: PRs #13–#26 are merged to `main`; this branch adds automated axe-core WCAG AA checks to the e2e suite, following through on the styling PR's own flagged limitation, on top of synchronized `main` (`73f339f`); generated browser results are ignored; no learner data, provider credentials, generated build output, or local database files are tracked
- Last verified commit before this branch: `73f339f fix: audit remaining upsert() calls for the identity-race class of bug (#26)`
...
- Added automated accessibility checks (`@axe-core/playwright`) to the e2e suite (`tests/browser/accessibility.spec.ts`), following through on the styling PR's own flagged limitation. Both pages pass zero violations against `wcag2a`/`wcag2aa`/`wcag21a`/`wcag21aa`/`wcag22aa`, including the parent page's dynamically-rendered digest content after clicking "Get weekly digest." Verified the test actually catches real problems, not just always-passes: deliberately broke the answer-input label association and confirmed axe failed the test with the correct WCAG 4.1.2 violation, then restored it and confirmed clean again. This is still only an **automated** check — axe-core catches roughly 30–50% of real WCAG issues (things like keyboard-only navigation flow, screen-reader announcement quality, and cognitive-load/plain-language criteria need manual review); it is not equivalent to a full manual accessibility audit.
...
- Uncommitted changes: `@axe-core/playwright` dev dependency and `tests/browser/accessibility.spec.ts`, on `feature/automated-accessibility-checks` off synchronized `main` (`73f339f`). `npm run verify` and `npm run test:e2e` both pass (5 browser tests total); verified the new check catches real violations, not just passing trivially.
...
- Next exact prompt: `Start from synchronized main. Automated WCAG AA checks now run in the e2e suite and both pages pass with zero detected violations, but this is only automated coverage (axe-core catches roughly 30-50% of real accessibility issues) — a manual accessibility review remains open. Remaining known gaps: no diagnostic/placement logic for which item a learner starts on (the server still defaults to unit-rates-1 regardless of the planner's top recommendation); no scheduler driving the plan or the digest automatically (deferred by design); no rubric-based validator type, so distributions/center-and-variability content was scoped to short deterministic answers instead of the open-ended descriptions their masteryCheckRule describes. Before adding real authentication, real learner data, or provider integration, resolve and record the pending hosting/authentication, learner identity/guardian verification, consent/retention, and model-provider/data-processing decisions.`
