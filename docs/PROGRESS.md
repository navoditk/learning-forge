...
- Phase: 1 — synthetic journeys across all 5 Grade 6 Math domains, skill graph, planner, an actionable planner UI, and an on-demand parent weekly digest covered; content is reviewed and approved (scope noted below); real identity/provider approvals remain pending
- Branch: `feature/weekly-digest-route-and-ui`
- Repository state: PRs #13–#23 are merged to `main`; this branch wires `NotifierPort` into a real on-demand digest route and parent-page button (still no scheduler), on top of synchronized `main` (`7c93423`); generated browser results are ignored; no learner data, provider credentials, generated build output, or local database files are tracked
- Last verified commit before this branch: `7c93423 chore: record product/content owner approval of all 38 content records (#23)`
...
- Uncommitted changes: generalized `WeeklyDigestInput`/`buildWeeklyDigest` to multi-skill, `getWeeklyDigest`, `GET /api/phase1/digest`, a parent-page "Get weekly digest" button, updated tests, and this progress update, on `feature/weekly-digest-route-and-ui` off synchronized `main` (`7c93423`). `npm run verify`, `npm run test:integration`, and `npm run test:e2e` all pass.
