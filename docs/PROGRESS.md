...
- Phase: 1 — synthetic Ratios+Number System+Expressions+Geometry journeys, skill graph, planner, and an actionable planner UI covered; real identity/provider approvals remain pending
- Branch: `feature/geometry-and-remaining-expressions-content`
- Repository state: PRs #13–#20 are merged to `main`; this branch completes Expressions and Equations and all of Geometry content, on top of synchronized `main` (`511137e`); generated browser results are ignored; no learner data, provider credentials, generated build output, or local database files are tracked
- Last verified commit before this branch: `511137e feat: complete Number System content, start Expressions and Equations (#20)`
...
| `ContentItem`, `Problem`, `HintStep`, `Rubric`, `ContentVersion` | Not implemented | Content lives as version-controlled JSON in `content/ratios`, `content/number-system`, `content/expressions-and-equations`, and `content/geometry`, validated by Zod, not a DB table. Content `skillCode` values are cross-checked against the skill catalog |
...
- 3 of the 19 catalog skills still have no authored content (`statistical-questions`, `distributions`, `center-and-variability` — all of Statistics); `planNextActivities` reports these honestly as `unavailableSkills` rather than inventing placeholder content.
...
- Uncommitted changes: 4 more Expressions and Equations records (`content/expressions-and-equations/one-variable-equations-and-inequalities-*`, `dependent-and-independent-variables-*`) and 6 new Geometry records (`content/geometry/`), updated tests, and this progress update, on `feature/geometry-and-remaining-expressions-content` off synchronized `main` (`511137e`). `npm run verify`, `npm run test:integration`, and `npm run test:e2e` all pass.
- Next exact prompt: `Start from synchronized main. Ratios, Number System, Expressions and Equations, and Geometry are fully covered (16 of 19 skills); only Statistics (statistical-questions, distributions, center-and-variability) has zero content. None of the authored content is human-reviewed yet (all pending_review). There is still no diagnostic/placement logic for which item a learner starts on or a scheduler driving the plan/NotifierPort digest. Before adding real authentication, real learner data, or provider integration, resolve and record the pending hosting/authentication, learner identity/guardian verification, consent/retention, and model-provider/data-processing decisions.`
