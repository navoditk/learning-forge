...
- Phase: 1 — synthetic Ratios+Number System+partial Expressions journeys, skill graph, planner, and an actionable planner UI covered; real identity/provider approvals remain pending
- Branch: `feature/expressions-and-number-system-content`
- Repository state: PRs #13–#19 are merged to `main`; this branch completes Number System content and starts Expressions and Equations content, on top of synchronized `main` (`da8ab56`); generated browser results are ignored; no learner data, provider credentials, generated build output, or local database files are tracked
- Last verified commit before this branch: `da8ab56 feat: author first Number System content, fix llm_drafted catalog gate (#19)`
...
| `ContentItem`, `Problem`, `HintStep`, `Rubric`, `ContentVersion` | Not implemented | Content lives as version-controlled JSON in `content/ratios`, `content/number-system`, and `content/expressions-and-equations`, validated by Zod, not a DB table. Content `skillCode` values are cross-checked against the skill catalog |
...
- 8 of the 19 catalog skills still have no authored content (`one-variable-equations-and-inequalities`, `dependent-and-independent-variables`, and all of Geometry and Statistics); `planNextActivities` reports these honestly as `unavailableSkills` rather than inventing placeholder content.
...
- Uncommitted changes: 4 more content records (`content/number-system/division-of-fractions-*`, `content/number-system/coordinate-plane-*`) and 4 new Expressions and Equations records (`content/expressions-and-equations/`), updated tests, and this progress update, on `feature/expressions-and-number-system-content` off synchronized `main` (`da8ab56`). `npm run verify`, `npm run test:integration`, and `npm run test:e2e` all pass.
- Next exact prompt: `Start from synchronized main. Number System is fully covered; Expressions and Equations has 2 of 4 skills covered (one-variable-equations-and-inequalities and dependent-and-independent-variables still have none); Geometry and Statistics remain entirely uncovered. None of the authored content is human-reviewed yet (all pending_review). There is still no diagnostic/placement logic for which item a learner starts on or a scheduler driving the plan/NotifierPort digest. Before adding real authentication, real learner data, or provider integration, resolve and record the pending hosting/authentication, learner identity/guardian verification, consent/retention, and model-provider/data-processing decisions.`
