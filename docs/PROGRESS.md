...
- Branch: `feature/mastery-upsert-race-audit`
- Repository state: PRs #13–#25 are merged to `main`; this branch follows through on an explicitly flagged risk by auditing every other `upsert()` call for the same class of concurrency race, on top of synchronized `main` (`fee8230`); generated browser results are ignored; no learner data, provider credentials, generated build output, or local database files are tracked
- Last verified commit before this branch: `fee8230 feat: add basic accessible styling; fix real identity-bootstrap race (#25)`
...
- Uncommitted changes: `src/server/prisma-errors.ts` (new, shared `isUniqueConstraintViolation`), the `masteryEstimate.upsert` retry-as-update fix in `createAttempt`, and a new concurrency test, on `feature/mastery-upsert-race-audit` off synchronized `main` (`fee8230`). `npm run verify`, `npm run test:integration`, and `npm run test:e2e` all pass.
