...
- Phase: 1 — synthetic journeys across all 5 Grade 6 Math domains, skill graph, planner, an actionable planner UI, an on-demand parent weekly digest, and a basic accessible visual design covered; content is reviewed and approved (scope noted below); real identity/provider approvals remain pending
- Branch: `feature/basic-styling-and-identity-race-fix`
- Repository state: PRs #13–#24 are merged to `main`; this branch adds the first visual design pass and fixes a real concurrency bug found while testing it in a browser, on top of synchronized `main` (`2df184d`); generated browser results are ignored; no learner data, provider credentials, generated build output, or local database files are tracked
- Last verified commit before this branch: `2df184d feat: wire NotifierPort into an on-demand weekly digest (#24)`
...
- The visual design pass is basic: one hand-written stylesheet, a single centered container layout, and OS-preference dark mode only (no manual toggle). It is not a full design system and has not had a dedicated accessibility audit beyond the existing WCAG-oriented conventions (focus-visible outlines, contrast-conscious color choices, no color-only meaning).
- The `ensureSyntheticIdentity` race was real and pre-existing across every prior PR in this session; it happened not to surface because every previous integration test called it sequentially within one Vitest process. It was only found by literally loading the page in a browser, which is why the run/screenshot verification habit matters, not just automated tests. No other identity/session code path was audited for the same class of race.
...
- All 19 catalog skills now have authored content (2 items each, 38 total); `planNextActivities`'s `unavailableSkills` is empty from a fresh mastery state for the first time. This closes the content-authoring-bottleneck risk as a coverage gap — it does not mean any of it is approved (see below).
...
- Uncommitted changes: `src/app/globals.css` (new, basic accessible visual design) plus the `ensureSyntheticIdentity` concurrency fix and its regression test, on `feature/basic-styling-and-identity-race-fix` off synchronized `main` (`2df184d`). `npm run verify`, `npm run test:integration`, and `npm run test:e2e` all pass; verified visually via Playwright screenshots of both pages and a real two-concurrent-request repro against a fresh database.
- Milestone: all 19 catalog skills across all 5 Grade 6 Math domains have authored, **reviewed** content (38 records, approved 2026-09-06 by the product/content owner — see ADR-0001's approval record for the explicit scope note, since it was not a separately engaged subject-matter-expert audit). `NotifierPort` now has a real, on-demand caller (still no scheduler).
- Next exact prompt: `Start from synchronized main. The app now has a basic accessible visual design and a real identity-bootstrap race condition is fixed (found by actually loading the page in a browser during the styling work, not by any prior test). Remaining known gaps: no diagnostic/placement logic for which item a learner starts on (the server still defaults to unit-rates-1 regardless of the planner's top recommendation); no scheduler driving the plan or the digest automatically (deferred by design); no rubric-based validator type, so distributions/center-and-variability content was scoped to short deterministic answers instead of the open-ended descriptions their masteryCheckRule describes; the visual design is a first pass only, not audited for accessibility beyond existing conventions. Before adding real authentication, real learner data, or provider integration, resolve and record the pending hosting/authentication, learner identity/guardian verification, consent/retention, and model-provider/data-processing decisions.`
