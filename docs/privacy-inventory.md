...
|---|---|---|---|---|---|---|
| Account identity | Link an approved account to a household; use provider subject and internal ID only | Parent for own household; identity module; support only when approved | Managed identity provider and app database; region not selected | Final schedule pending privacy/legal approval; revoke access and delete linkage on approved request | Account and consent metadata | No provider selected; never log credentials or tokens |
